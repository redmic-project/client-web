define([
	...
	, 'src/component/mapLayer/detection/DetectionLayerImpl'
], function(
	declare
	, moment
	, _RequestData
	...
	return declare(null, {
		// summary:
		//   Lógica de diseño para añadir una colección de componentes DetectionMapLayer, para representar datos de
		//   detección sobre el mapa.
		...
		_setConfigurations: function() {
			...
			this.mergeComponentAttribute('detectionMapLayerConfig', {
				parentChannel,
		...
		createDesignControllerComponents: function() {
			...
			this._DetectionMapLayer = this._defineDesignDetectionMapLayerComponent(mapChannel);

			return inheritedComponents;
		},

		_defineDesignDetectionMapLayerComponent: function(mapChannel) {

			this.mergeComponentAttribute('detectionMapLayerConfig', {
				mapChannel
			});

			return declare([DetectionLayerImpl, _RequestData, _PublishInfo, _ListenZoom]);
		},
		...
		_setOwnCallbacksForEvents: function() {
			...
			this.on('ME_OR_ANCESTOR_HIDDEN', () => this._addDetectionMapLayerComponentsOnHide());
		},
		...
		_createLayerInstance: function(item) {
			...
			const layerConfig = this._merge([this.detectionMapLayerConfig, specificLayerConfig]);
			return new this._DetectionMapLayer(layerConfig);
		},

		_generateLayerId: function(uuid) {

			return `detection_${uuid}`;
		},
