define([
	...
	, 'src/component/mapLayer/detection/_DetectionDataManagement'
	, 'src/component/mapLayer/detection/_DetectionMarkersManagement'
	, 'RWidgets/Utilities'
], function(
	d3
	, declare
	...
	, Deferred
	, _Module
	, _DetectionDataManagement
	, _DetectionMarkersManagement
	, Utilities
) {

	return declare([_Module, _DetectionDataManagement, _DetectionMarkersManagement], {
		//	summary:
		...
		_subAddData: function(req) {
			...
			features?.forEach(feature => this._addData(feature));
		...
		_drawUntilPosition: function(position, animate) {
			...
			let steps = position;
			if (Utilities.isValidNumber(this._lastPosition) && !this._checkDateIsValid(this._lastPosition)) {
			...
			this._updateMarkerByPosition(clusterPosition);
		...
		_jumpToPast: function(pos) {
			...
			var endPosition = this._getFeatureIndexByPosition(pos),
