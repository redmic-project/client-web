define([
	...
	return declare([Layout, Controller, _Main, _AddTitle, _TitleSelection], {
		...
		constructor: function(args) {
			...
			this.target = redmicConfig.services.project;
			this.reportService = "project";
				...
				additionalInfo: {
					...
					props: {
						...
						childTabs: [{
							...
							type: declare([ListImpl, _Framework, _Select, _Filter]),
							props: {
		...
		_refreshChildrenDataModules: function() {
			...
			var projectChildrenTarget = lang.replace(redmicConfig.services.activityProject, {
				id: this.pathVariableId
			});

			this._publish(this._widgets.additionalInfo.getChildChannel("childInstances.4", "UPDATE_TARGET"), {
				target: projectChildrenTarget,
				refresh: true
			});
		},
